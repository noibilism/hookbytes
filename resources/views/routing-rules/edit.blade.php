...
@section('title', 'Edit Routing Rule - HookBytes Dashboard')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="border-b border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">Edit Routing Rule</h1>
        ...
        <!-- Form -->
        <form method="POST" action="{{ route('routing-rules.update', [$endpoint, $routingRule]) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $routingRule->name) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="priority" id="priority" value="{{ old('priority', $routingRule->priority) }}"
                               min="1" max="100"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="2"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Optional description of what this rule does">{{ old('description', $routingRule->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center">
                            <input type="radio" name="action" value="route" 
                                   {{ old('action', $routingRule->action) === 'route' ? 'checked' : '' }}
                        ...
                        <label class="flex items-center">
                            <input type="radio" name="action" value="drop" 
                                   {{ old('action', $routingRule->action) === 'drop' ? 'checked' : '' }}
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                                   onchange="toggleDestinations()">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" 
                           {{ old('is_active', $routingRule->is_active) ? 'checked' : '' }}
            ...
            <div id="destinations-section" class="mt-8">
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div id="destinations-container">
                        <!-- Existing destinations will be loaded here -->
                    </div>
...
const existingConditions = <?php echo json_encode(old('conditions', $routingRule->conditions ?? [])); ?>;
const existingDestinations = <?php echo json_encode(old('destinations', $routingRule->destinations ?? [])); ?>;
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="conditions[${conditionIndex}][operator]" 
                        ...
                        <option value="not_exists" ${operator === 'not_exists' ? 'selected' : ''}>not exists</option>
                    </select>
                </div>
                <div>
...
function addDestination(url = '', priority = 10) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (existingConditions && existingConditions.length > 0) {
        ...
        // Add one empty condition if none exist
        addCondition();
    }
    ...
    if (existingDestinations && existingDestinations.length > 0) {
        existingDestinations.forEach(destination => {
            addDestination(destination.url || '', destination.priority || 10);
        });
    } else {
        // Add one empty destination if action is route and none exist
